...
class VolumeEstimator():
    ...
    def estimate_volume(self, input_image, fov=70,  plate_diameter_prior=0.3,
        ...
        if (any(x != 0 for x in ellipse_params_scaled) and
            ...
            # the plate contour
            # 수정 print('[*] No ellipse found. Scaling with expected median depth.')
        ...
        for k in range(masks_array.shape[-1]):
            ...
            plane_params = pca_plane_estimation(object_points_filtered) # 좌표 임의로 지정하는 방법 plane_params = [w0, w1, w2, w3]
            # print('플레인',plane_params)
            # plane_params = [0.5, -0.5, -0.5, 0.5]
            ...
            if (plots_directory is not None): # 수정 (plot_results or plots_directory is not None)
    ...
    def __create_intrinsics_matrix(self, input_image_shape, fov):
        ...
        # 수정 print('[*] Creating intrinsics matrix from given FOV:', fov)
...
def masking(img, bndbox):
    try:
        ...
        masked_img[:y_min, :] = 255
        masked_img[y_max:, :] = 255
        masked_img[:, :x_min] = 255
        masked_img[:, x_max:] = 255
        return masked_img
